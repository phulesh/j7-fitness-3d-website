...
export function coverSvg(opts: {
  ...
  const title = wrapText(opts.title, opts.title.length > 40 ? 18 : 14).slice(0, 5);
  const titleSize = opts.title.length > 48 ? 42 : opts.title.length > 28 ? 52 : 64;
...
<svg xmlns="http://www.w3.org/2000/svg" width="800" height="1200" viewBox="0 0 800 1200" xml:lang="${opts.language}">
  ...
  ${ornament}
  <text x="72" y="96" fill="${p.accent}" font-family="Georgia, 'Source Serif 4', serif" font-size="14" letter-spacing="4" text-transform="uppercase">${escapeXml(opts.style.toUpperCase())} · FOLIO</text>
  <text x="72" y="280" fill="${p.fg}" font-family="Georgia, 'Source Serif 4', serif" font-size="${titleSize}" font-weight="600" direction="${dir}">${titleTspans}</text>
  <text x="72" y="${280 + title.length * (titleSize + 8) + 36}" fill="${p.muted}" font-family="Georgia, serif" font-size="22">
