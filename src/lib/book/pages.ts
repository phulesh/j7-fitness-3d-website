...
export function buildBookPages(doc: EbookDocument): BookPage[] {
  ...
  for (const [i, ch] of doc.chapters.entries()) {
    const body = [
      ...
      ...(ch.sections.some((s) => /ebook-figure/.test(s.html))
        ? []
        : (ch.images || []).map(
            (img) =>
              `<figure class="ebook-figure"><img src="${esc(img.url)}" alt="${esc(img.alt || img.caption)}" /><p><strong>${esc(
                img.figureLabel || img.caption
              )}</strong></p><p>${esc(img.credit)}</p>${
                img.verifiedHistoricalPhoto === false ? "<p>व्याख्यात्मक चित्र — यह ऐतिहासिक फोटोग्राफ नहीं है।</p>" : ""
              }</figure>`
          )),
      ch.keyPoints.length
